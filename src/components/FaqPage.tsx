  ...
  return (
    <div className=''>
        <div className="flex flex-col space-y-3 mx-auto w-[90%] md:w-3/4 lg:w-2/4 text-center my-5">
            <span className="text-black text-[28px] md:text-[40px] font-bold">Frequently Asked Questions</span>
        ...
        <div className='w-[90%] md:w-3/4 lg:w-3/5 my-10 mx-auto'>
            <Accordion type="single" collapsible className="w-full">
                {
                    faqs.map((faq,index)=>(
                        <AccordionItem key={faq.id} value={`item-${index}`}>
                            <AccordionTrigger className='text-sm md:text-xl'>{faq.title}</AccordionTrigger>
                            <AccordionContent className='text-xs md:text-md'>
                            {faq.desc}
                            </AccordionContent>
                        </AccordionItem>
                    ))
