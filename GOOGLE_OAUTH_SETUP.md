...
1. Find your OAuth 2.0 Client ID: `YOUR_GOOGLE_CLIENT_ID`
...
> **Important**: Never commit actual credentials to version control. Always use environment variables and add `.env` to your `.gitignore` file.
...
- **Client ID**: Retrieved from `VITE_GOOGLE_CLIENT_ID` environment variable
...
2. **Use .env files**: Store credentials in `.env` files that are excluded from version control
3. **Rotate credentials**: If credentials are accidentally exposed, rotate them immediately in Google Cloud Console
4. **Limit redirect URIs**: Only add the specific URIs you need for your application
